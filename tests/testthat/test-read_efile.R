test_that("read_gov_grants_raw normalizes EIN and types", {
...
test_that("read_pf_pri_raw normalizes EIN and types", {
  tmp <- tempfile(fileext = ".parquet")
  arrow::write_parquet(
    tibble::tibble(
...
test_that("read_gov_grants_raw errors on a missing expected column", {
  tmp <- tempfile(fileext = ".parquet")
  arrow::write_parquet(tibble::tibble(ein = "123456789", tax_year = 2021), tmp)
  expect_error(read_gov_grants_raw(tmp), "government_grants")
})
